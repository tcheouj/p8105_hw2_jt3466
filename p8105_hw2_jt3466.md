...
robots and all time points is 2.269548^{6} bottles), `polystyrene`,
...
be matched. Then, the `anti_join`s for the `bakes` and `results`, and
`bakers` and `results` dataframes are run again to ensure this has been
fixed and Jo Wheatley’s information across the datasets has been
successfully merged. These `anti_join`s should then print no
