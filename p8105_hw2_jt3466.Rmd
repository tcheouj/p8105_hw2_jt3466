...
```{r q2}
mr_trash <- read_excel("data/202409 Trash Wheel Collection Data.xlsx", range = "A2:N653", sheet = "Mr. Trash Wheel") |>
  janitor::clean_names() |>
  mutate(
...
prof_trash <- read_excel("data/202409 Trash Wheel Collection Data.xlsx", range = "A2:M120", sheet = "Professor Trash Wheel") |>
  janitor::clean_names() |>
  mutate(robot = "Professor Trash Wheel")

gwynnda <- read_excel("data/202409 Trash Wheel Collection Data.xlsx", range = "A2:L265", sheet = "Gwynnda Trash Wheel") |>
  janitor::clean_names() |>
...
In addition, the `anti_join` for `bakers` and `bakes` has rows from season 9 and 10, as the `bakes` dataset is missing data from those seasons. It stops at season 8. 
...
Worth noting, the baker `Joanne` has `NA`s throughout, as her corresponding information did not join as she may be named `Jo` in the `bakers` table and `"Jo"` in the `bakes` table. 
